...
theorem cofinal_affineSupportGrowth_has_boundedPrefixInjury
  ...
  | succ k ih =>
      intro d c W hgrowth
      let SafeAt : ℕ → Prop := fun b =>
        ∃ E ∈ additiveSupportFamily A (k + 1) (d + b - c),
          b ∉ E ∧
          Disjoint (E : Set ℕ) (W : Set ℕ)
      by_cases hsafeCofinal :
      ...
      · obtain ⟨L₀, hL₀⟩ := not_forall.mp hsafeCofinal
        let anchorBound :=
          ∑ t ∈ Finset.range (d + 1),
            (additiveSupportFamily A k t).card
        have hdiagonal :
            ∀ X, ∃ y, X < y ∧
              ∃ w ∈ W, w ∈ A ∧ ∃ b,
                y ≤ b ∧ b ∈ K ∧
          ...
          let Hb : Finset (Finset ℕ) :=
            H.filter fun E => b ∈ E
          let H₀ : Finset (Finset ℕ) :=
            H.filter fun E => b ∉ E
          have hHbSub :
              Hb ⊆ additiveSupportFamily A (k + 1) (d + b - c) := by
            intro E hEHb
            exact (Finset.mem_filter.mp hEHb).1
          have hbHb : ∀ E ∈ Hb, b ∈ E := by
            intro E hEHb
            exact (Finset.mem_filter.mp hEHb).2
          have hHbBound : Hb.card ≤ anchorBound := by
            by_cases hHbEmpty : Hb = ∅
            · simp [hHbEmpty]
            · have hHbNonempty : Hb.Nonempty :=
                Finset.nonempty_iff_ne_empty.mpr hHbEmpty
              obtain ⟨E, hEHb⟩ := hHbNonempty
              have hER := hHbSub hEHb
              have hbE := hbHb E hEHb
              have hbTarget : b ≤ d + b - c :=
                additiveSupportFamily_supportsBounded
                  A (k + 1) (d + b - c) E hER b hbE
              obtain ⟨lower, hlowerSub, hlowerCard⟩ :=
                additiveSupportStar_descends_card hHbSub hbHb
              have htargetSmall : d + b - c - b < d + 1 := by
                omega
              have hfamilyBound :
                  (additiveSupportFamily A k
                    (d + b - c - b)).card ≤ anchorBound := by
                dsimp only [anchorBound]
                exact Finset.single_le_sum
                  (f := fun t =>
                    (additiveSupportFamily A k t).card)
                  (fun _ _ => Nat.zero_le _)
                  (Finset.mem_range.mpr htargetSmall)
              calc
                Hb.card = lower.card := hlowerCard.symm
                _ ≤
                    (additiveSupportFamily A k
                      (d + b - c - b)).card :=
                  Finset.card_le_card hlowerSub
                _ ≤ anchorBound := hfamilyBound
          have hsplit : Hb.card + H₀.card = H.card := by
            simpa only [Hb, H₀] using
              (Finset.card_filter_add_card_filter_not
                (s := H) (p := fun E => b ∈ E))
          have hH₀large : W.card * (y + 1) < H₀.card := by
            dsimp only [demand] at hbLarge
            dsimp only [H] at hsplit
            omega
          have hH₀Sub :
              H₀ ⊆ additiveSupportFamily A (k + 1) (d + b - c) := by
            intro E hEH₀
            exact (Finset.mem_filter.mp hEH₀).1
          have hbH₀ : ∀ E ∈ H₀, b ∉ E := by
            intro E hEH₀
            exact (Finset.mem_filter.mp hEH₀).2
          have hhitW :
              ∀ E ∈ H₀,
                ¬ Disjoint (E : Set ℕ) (W : Set ℕ) := by
            intro E hEH₀ hEW
            exact hnotSafe
              ⟨E, hH₀Sub hEH₀, hbH₀ E hEH₀, hEW⟩
          have hH₀Nonempty : H₀.Nonempty := by
            rw [← Finset.card_pos]
            exact lt_of_le_of_lt (Nat.zero_le _) hH₀large
          let default : ℕ := 0
          let hit : Finset ℕ → ℕ := fun E =>
            if hE : E ∈ H₀ then
              Classical.choose
                (Set.not_disjoint_iff.mp (hhitW E hE))
            else default
          have hhitE : ∀ E ∈ H₀, hit E ∈ E := by
            intro E hEH
            simp only [hit, dif_pos hEH]
            exact Finset.mem_coe.mp
              (Classical.choose_spec
                (Set.not_disjoint_iff.mp (hhitW E hEH))).1
          have hhitW' : ∀ E ∈ H₀, hit E ∈ W := by
            intro E hEH
            simp only [hit, dif_pos hEH]
            exact Finset.mem_coe.mp
              (Classical.choose_spec
                (Set.not_disjoint_iff.mp (hhitW E hEH))).2
          obtain ⟨w, hwW, hwFiber⟩ :=
            Finset.exists_lt_card_fiber_of_mul_lt_card_of_maps_to
              hhitW' hH₀large
          let Hw : Finset (Finset ℕ) :=
            H₀.filter fun E => hit E = w
          have hHwSub :
              Hw ⊆ additiveSupportFamily A (k + 1) (d + b - c) := by
            intro E hEHw
            exact hH₀Sub (Finset.mem_filter.mp hEHw).1
          have hwHw : ∀ E ∈ Hw, w ∈ E := by
            intro E hEHw
            obtain ⟨hEH₀, hhit⟩ := Finset.mem_filter.mp hEHw
            simpa only [hhit] using hhitE E hEH₀
          obtain ⟨lower, hlowerSub, hlowerCard⟩ :=
            additiveSupportStar_descends_card hHwSub hwHw
          have hHwNonempty : Hw.Nonempty := by
            rw [← Finset.card_pos]
            exact lt_of_le_of_lt (Nat.zero_le _) hwFiber
          obtain ⟨E, hEHw⟩ := hHwNonempty
          have hwA :
              w ∈ A :=
            additiveSupportFamily_supportsIn
              A (k + 1) (d + b - c) E
                (hHwSub hEHw) w (hwHw E hEHw)
          have hlowerLarge :
              y <
                (additiveSupportFamily A k
                  (d + b - (c + w))).card := by
        ...
        have hwA : w ∈ A := by
          obtain ⟨y, _hy, hwA, _b, _hyb, _hbA, _hgrowth⟩ :=
            hwCofinal 0
          exact hwA
        have hlowerGrowth :
            ∀ r L, ∃ b,
        ...
        refine ⟨insert w S, ?_, ?_, ?_, ?_⟩
        · intro x hx
          rcases Finset.mem_insert.mp hx with rfl | hxS
          · exact hwW
          · exact Finset.mem_of_mem_erase (hSW hxS)
        · have hwS : w ∉ S := by
            intro hwS'
            exact (Finset.mem_erase.mp (hSW hwS')).1 rfl
          rw [Finset.card_insert_of_notMem hwS]
          omega
        · intro x hx
          rcases Finset.mem_insert.mp hx with rfl | hxS
          · exact hwA
        ...
        · intro L
          ...
          have hcw : c + w < b :=
            (le_max_right L (c + w + 1)).trans hbLower
          ...
          · intro hbInsert
            rcases Finset.mem_insert.mp hbInsert with hbw | hbE'
            · exact hwb hbw.symm
            · exact hbE hbE'
          · rw [Set.disjoint_left]
            intro x hxInsert hxRetained
            have hxRetainedFin :
                x ∈ W \ insert w S :=
              Finset.mem_coe.mp hxRetained
            have hxW : x ∈ W :=
              (Finset.mem_sdiff.mp hxRetainedFin).1
            have hxNotInsert : x ∉ insert w S :=
              (Finset.mem_sdiff.mp hxRetainedFin).2
            rcases Finset.mem_insert.mp
                (Finset.mem_coe.mp hxInsert) with rfl | hxE
            · exact hxNotInsert (Finset.mem_insert_self _ S)
            · have hxErase : x ∈ W.erase w := by
                apply Finset.mem_erase.mpr
                exact ⟨fun hxw => hxNotInsert
                  (hxw ▸ Finset.mem_insert_self w S), hxW⟩
              have hxNotS : x ∉ S := by
                intro hxS
                exact hxNotInsert (Finset.mem_insert_of_mem hxS)
              exact Set.disjoint_left.mp hEavoid
                (Finset.mem_coe.mpr hxE)
                (Finset.mem_coe.mpr
...
theorem IsExactTupleAsymptoticBasis.fixedSuccessorTranslate_boundedInjuryInfiniteDeletion
  ...
  obtain ⟨B, hBA, hBInfinite, hBRetained, hrepair⟩ :=
    cofinal_anchorAvoidingSupports_avoidingFinite_have_infiniteFreeDeletion
      ...
      (by
        intro L
        obtain ⟨b, E, hbLower, hbA, hER, hbE, hEretained⟩ :=
          hcofinal L
        exact ⟨b, E, hbLower, hbA, hER, hbE, hEretained⟩)
...
theorem exactBasis_counterexample_forces_noncontainedFailedRepairs_or_pureDeletionRootedMatchings
  ...
  · right
    ...
    obtain ⟨U, hU⟩ :=
      hIFinite.exists_le
    refine ⟨⟨U, ?_⟩, ?_⟩
    · intro n hUn hnDestroy E hER x hxE
      by_contra hxB
      have hnI : n ∈ I :=
        ⟨hnDestroy, E, hER, x, hxE, hxB⟩
      exact (not_lt_of_ge (hU n hnI)) hUn
    · intro r L
...
theorem exactBasis_counterexample_forces_fixedReservoir_freshMatchings
  ...
  have hACB : A ⊆ C ∪ B := by
    intro x hxA
    obtain ⟨n, _hnFinite, hnx⟩ :=
      Nat.exists_lt_card_nth_eq hxA
    rw [← hnx]
...
theorem minimalAdditiveDestroyer_has_prefixClearedUniversalAnchorFan
      ...
      ∀ pads : List ℕ,
        ...
        ∃ repaired : Finset ℕ,
          repaired ∈
            additiveSupportFamily A H
              (pads.sum + t) ∧
          Disjoint (repaired : Set ℕ) (W : Set ℕ) ∧
  ...
  have hxHits : x ∈ hits := by
    ...
    · exact Set.disjoint_left.mp hcoreW
        ...
        (Finset.mem_coe.mpr (hDW hxD))
  refine ⟨hits, j, t, core, hhitsLength, hlength,
    hhitsW, ⟨x, hxD, hxHits⟩, htarget,
    hcoreR, hcoreW, ?_⟩
  ...
  have hrepairW :
    ...
    have hyUnion : y ∈ pads.toFinset ∪ core := by
      simpa [repaired, foldr_insert_eq_toFinset_union] using hyRepair
    rcases Finset.mem_union.mp hyUnion with hyPads | hyCore
    · exact (hpads y (List.mem_toFinset.mp hyPads)).2
        (Finset.mem_coe.mp hyW)
    · exact Set.disjoint_left.mp hcoreW
        (Finset.mem_coe.mpr hyCore) hyW
...
This converts the arithmetic non-normalization left by the universal tuple
fan into the lower-rank support-family language used by the matching and
rank-descent engines. -/
theorem no_cleanTupleNormalization_forces_partitionDestroyer
    {A C B : Set ℕ} {ℓ n : ℕ} {F : Finset ℕ}
    (hACB : A ⊆ C ∪ B)
    (hno : ∀ pads : List ℕ,
      pads.length = ℓ →
      (∀ a ∈ pads, a ∈ C ∧ a ∉ F) →
  ...
  rintro ⟨v, hv, hvsum⟩
  let pads : List ℕ := List.ofFn v
  have hpadsLength : pads.length = ℓ := by
    simp [pads]
  have hpadsClean : ∀ a ∈ pads, a ∈ C ∧ a ∉ F := by
    intro a haPads
    obtain ⟨i, hia⟩ := List.mem_ofFn.mp haPads
    have hvi := hv i
    have haA : a ∈ A := by
      simpa only [hia] using hvi.1
    have haNotUnion : a ∉ B ∪ (F : Set ℕ) := by
      simpa only [hia] using hvi.2
    constructor
    · rcases hACB haA with haC | haB
      · exact haC
      · exact (haNotUnion (Or.inl haB)).elim
    · intro haF
      exact haNotUnion
        (Or.inr (Finset.mem_coe.mpr haF))
  exact (hno pads hpadsLength hpadsClean) <| by
    change (List.ofFn v).sum = n
    rw [List.sum_ofFn]
    exact hvsum
...
theorem complementarySurvivingCore_forces_hitTargetDestroyer
    ...
    (horder : ℓ + j = H)
    (htarget : q = n + t)
    (hcoreR : core ∈ additiveSupportFamily A j t)
    (hcoreD : Disjoint (core : Set ℕ) (D : Set ℕ)) :
    DestroysAt
      (additiveSupportFamily A ℓ)
      (D : Set ℕ) n := by
  classical
  ...
  have hpadsLength : pads.length = ℓ := by
    simp [pads]
  have hpadsSum : pads.sum = n := by
    change (List.ofFn fun i => (v i : ℕ)).sum = n
    rw [List.sum_ofFn]
    exact hvsum
  let repaired : Finset ℕ :=
  ...
  have hrepairQ :
      repaired ∈ additiveSupportFamily A H q := by
    have hrepairOrder : pads.length + j = H := by
      omega
    have hrepairTarget : pads.sum + t = q := by
      omega
    rw [hrepairOrder, hrepairTarget] at hrepairRaw
    exact hrepairRaw
  have hrepairD :
      Disjoint (repaired : Set ℕ) (D : Set ℕ) := by
    rw [Set.disjoint_left]
    intro z hzRepair hzD
    ...
    · obtain ⟨i, hiz⟩ :=
        List.mem_ofFn.mp (List.mem_toFinset.mp hzPads)
        ...
        ∃ core repaired : Finset ℕ,
          ...
          ∃ rank, 0 < rank ∧ rank ≤ h + 1 ∧
            ∃ m, ∃ M : Finset (Finset ℕ),
              ...
              r < M.card := by
  classical
  obtain ⟨C, B, hCA, hCInfinite, hBA, hBInfinite, hCB,
      _hACB, hstage⟩ :=
    exactBasis_counterexample_forces_fixedReservoir_freshMatchings
      hhpos hbasis hcounter
...
/-- One recursive minimal-destroyer block carrying a common-core repair
fan over the fixed anchor reservoir `C`. -/
structure FreshUniversalAnchorFanStep
    (A C B : Set ℕ) (H : ℕ) (used : Finset ℕ) (last : ℕ) where
  failure : ℕ
  block : Finset ℕ
  hits : List ℕ
  rank : ℕ
  ...
  block_minimal :
    IsInclusionMinimalDestroyer
      (additiveSupportFamily A H) block failure
  hits_nonempty : 0 < hits.length
  length_eq : hits.length + rank = H
  hits_mem :
    ∀ y ∈ hits, y ∈ A ∧ y ∈ block ∪ used
  block_hit : ∃ y ∈ block, y ∈ hits
  failure_eq : failure = hits.sum + residual
  core_mem :
    core ∈ additiveSupportFamily A rank residual
  core_fresh :
    Disjoint (core : Set ℕ)
      ((block ∪ used : Finset ℕ) : Set ℕ)
  tuple_fan :
...
theorem freshUniversalAnchorFanStep_nonempty
    ...
    (used : Finset ℕ) (last : ℕ) :
    Nonempty
      (FreshUniversalAnchorFanStep
        A C B H used last) := by
  obtain ⟨q, D, hq, hDnonempty, hDB, hDF, hminimal⟩ :=
    hfresh used (last + 1)
  obtain ⟨x, hxD⟩ := hDnonempty
  obtain ⟨hits, j, t, core, hhitsLength, hlength,
      hhitsW, hblockHit, htarget, hcoreR, hcoreW, hfan⟩ :=
    minimalAdditiveDestroyer_has_prefixClearedUniversalAnchorFan
      hminimal hxD Finset.subset_union_left hCA
  exact ⟨⟨q, D, hits, j, t, core, by omega,
    ⟨x, hxD⟩, hDB, hDF, hminimal, hhitsLength,
...
theorem exactBasis_counterexample_forces_coherentInfiniteUniversalAnchorFans
    ...
    ∃ C B K : Set ℕ,
      ...
      ∃ core forbidden : ℕ → Finset ℕ,
        ...
        (∀ i,
          DestroysAt
            (additiveSupportFamily A (hits i).length)
            (B ∪ (forbidden i : Set ℕ)) (hits i).sum) ∧
        (∀ i, ∀ a ∈ C, a ∉ forbidden i →
          ∃ E ∈ additiveSupportFamily A (h + 1)
              ((hits i).length * a + residual i),
            Disjoint (E : Set ℕ) K) ∧
        (∀ i L, ∃ a, a ∈ C ∧ L < a ∧
          a ∉ forbidden i) ∧
        (∃ ℓ, 0 < ℓ ∧ ℓ ≤ h + 1 ∧
  ...
  have hfresh : ∀ F : Finset ℕ, ∀ L,
      ∃ q, ∃ D : Finset ℕ,
        ...
        Disjoint D F ∧
        IsInclusionMinimalDestroyer
          (additiveSupportFamily A (h + 1)) D q := by
    intro F L
    obtain ⟨q, D, hLq, hDnonempty, hDB, hDF,
        hminimal, _hmatching⟩ :=
      hstage F 0 L
    exact ⟨q, D, hLq, hDnonempty, hDB, hDF, hminimal⟩
  let State := Finset ℕ × ℕ
  let initial : State := (∅, 0)
  let chooseStep : (i : ℕ) → (st : State) →
      FreshUniversalAnchorFanStep
        A C B (h + 1) st.1 st.2 :=
    fun _i st => Classical.choice
      (freshUniversalAnchorFanStep_nonempty
        hCA hfresh st.1 st.2)
  let advance : ℕ → State → State := fun i st =>
    (((st.1 ∪ (chooseStep i st).block) ∪
        (chooseStep i st).core),
  ...
  let residual (i : ℕ) : ℕ := (step i).residual
  let core (i : ℕ) : Finset ℕ := (step i).core
  let forbidden (i : ℕ) : Finset ℕ := block i ∪ used i
  have hstate_succ : ∀ i, state (i + 1) = advance i (state i) := by
    intro i
    simp [state]
  have hused_succ : ∀ i,
      used (i + 1) =
        (used i ∪ block i) ∪ core i := by
    intro i
    change (state (i + 1)).1 =
      (((state i).1 ∪ (chooseStep i (state i)).block) ∪
        (chooseStep i (state i)).core)
    rw [hstate_succ]
  have hlast_succ : ∀ i, (state (i + 1)).2 = failure i := by
    intro i
    change (state (i + 1)).2 =
  ...
  have hblock_into_next : ∀ i, block i ⊆ used (i + 1) := by
    ...
    exact Finset.mem_union_left _
      (Finset.mem_union_right _ hx)
  have hcore_into_next : ∀ i, core i ⊆ used (i + 1) := by
    intro i x hx
    rw [hused_succ]
    exact Finset.mem_union_right _ hx
  have hfailureStrict : StrictMono failure := by
  ...
  have hKInfinite : K.Infinite := by
    ...
    rintro x ⟨i, rfl⟩
    exact ⟨i, hpointBlock i⟩
  let hitPoint (i : ℕ) := (step i).block_hit.choose
  have hhitPointBlock : ∀ i, hitPoint i ∈ block i :=
    fun i => (step i).block_hit.choose_spec.1
  have hhitPointHits : ∀ i, hitPoint i ∈ hits i :=
    fun i => (step i).block_hit.choose_spec.2
  have hhitPointInjective : Function.Injective hitPoint := by
    intro i j hpoint
    by_contra hij
    exact Finset.disjoint_left.mp (hblockPairwise hij)
      (hhitPointBlock i) (hpoint ▸ hhitPointBlock j)
  have hcoreK : ∀ i, Disjoint (core i : Set ℕ) K := by
    intro i
    rw [Set.disjoint_left]
    rintro x hxCore ⟨j, hxBlock⟩
    rcases lt_trichotomy j i with hji | hji | hij
    · have hxUsed : x ∈ used i :=
        hused_mono (Nat.succ_le_of_lt hji)
          (hblock_into_next j hxBlock)
  ...
  have htupleK : ∀ i, ∀ pads : List ℕ,
      ...
      ∃ E ∈ additiveSupportFamily A (h + 1)
          (pads.sum + residual i),
        Disjoint (E : Set ℕ) K := by
    intro i pads hpadsLength hpads
    obtain ⟨E, hER, _hEW, hEsub⟩ :=
      (step i).tuple_fan pads hpadsLength hpads
    refine ⟨E, hER, ?_⟩
    rw [Set.disjoint_left]
    intro x hxE hxK
    rcases Finset.mem_union.mp
        (hEsub (Finset.mem_coe.mp hxE)) with hxPads | hxCore
    · have hxC : x ∈ C :=
        (hpads x (List.mem_toFinset.mp hxPads)).1
      exact Set.disjoint_left.mp hCB hxC (hKB hxK)
    · exact Set.disjoint_left.mp (hcoreK i)
        (Finset.mem_coe.mpr hxCore) hxK
  have hfanK : ∀ i, ∀ a ∈ C, a ∉ forbidden i →
      ∃ E ∈ additiveSupportFamily A (h + 1)
          ((hits i).length * a + residual i),
        Disjoint (E : Set ℕ) K := by
    intro i a haC haForbidden
    let pads := List.replicate (hits i).length a
    obtain ⟨E, hER, hEK⟩ :=
      htupleK i pads (by simp [pads]) (by
        intro x hxPads
        have hxa : x = a :=
          (List.mem_replicate.mp hxPads).2
        simpa only [hxa] using ⟨haC, haForbidden⟩)
    refine ⟨E, ?_, hEK⟩
    simpa [pads] using hER
  have hfreshAnchor : ∀ i L, ∃ a, a ∈ C ∧ L < a ∧
      a ∉ forbidden i := by
    intro i L
    obtain ⟨a, haC, haLarge⟩ :=
      hCInfinite.exists_gt
        (max L ((forbidden i).sup id))
    refine ⟨a, haC,
      (le_max_left L ((forbidden i).sup id)).trans_lt haLarge, ?_⟩
    intro haForbidden
    have haSup : a ≤ (forbidden i).sup id :=
      Finset.le_sup (f := id) haForbidden
    exact (not_lt_of_ge haSup)
      ((le_max_right L ((forbidden i).sup id)).trans_lt haLarge)
  have hdestroyK : ∀ i,
      DestroysAt
        (additiveSupportFamily A (h + 1)) K (failure i) := by
    intro i
    exact (step i).block_minimal.1.mono <| by
      intro x hx
      exact ⟨i, Finset.mem_coe.mp hx⟩
  have hnoCleanNormalization : ∀ i, ∀ pads : List ℕ,
      pads.length = (hits i).length →
      (∀ a ∈ pads, a ∈ C ∧ a ∉ forbidden i) →
      pads.sum ≠ (hits i).sum := by
    intro i pads hpadsLength hpads hsum
    obtain ⟨E, hER, hEK⟩ :=
      htupleK i pads hpadsLength hpads
    have hEFailure :
        E ∈ additiveSupportFamily A (h + 1) (failure i) := by
      have hfailureEq := (step i).failure_eq
      change failure i = (hits i).sum + residual i at hfailureEq
      rw [hfailureEq, ← hsum]
      exact hER
    exact (hdestroyK i E hEFailure) hEK
  have hlowerDestroy : ∀ i,
      DestroysAt
        (additiveSupportFamily A (hits i).length)
        (B ∪ (forbidden i : Set ℕ)) (hits i).sum := by
    intro i
  ...
  let slope : ℕ → Fin (h + 2) := fun i =>
    ⟨(hits i).length, by
      ...
      change (hits i).length + rank i = h + 1 at this
      omega⟩
  obtain ⟨ℓ, hℓFiber⟩ :=
    Finite.exists_infinite_fiber slope
  have hfiberEq :
      slope ⁻¹' {ℓ} = {i | (hits i).length = ℓ.1} := by
    ext i
    simp only [Set.mem_preimage, Set.mem_singleton_iff,
      Set.mem_setOf_eq]
    constructor
  ...
  have hfixedSlopeHitSums :
      ∀ L, ∃ i, L < (hits i).sum ∧
        ...
        DestroysAt
          ...
          (B ∪ (forbidden i : Set ℕ)) (hits i).sum := by
    have himageInfinite :
        (hitPoint '' {i | (hits i).length = ℓ.1}).Infinite :=
      (Set.infinite_image_iff hhitPointInjective.injOn).mpr
        hfixedSlope
    intro L
    obtain ⟨x, ⟨i, hiSlope, rfl⟩, hLx⟩ :=
      himageInfinite.exists_gt L
    refine ⟨i, hLx.trans_le ?_, hiSlope, ?_⟩
    · exact List.le_sum_of_mem (hhitPointHits i)
  ...
  refine ⟨C, B, K, block, failure, hits, rank, residual,
    ...
    (fun i => (step i).block_nonempty),
    hblockPairwise, hfailureStrict,
    (fun i => (step i).block_minimal),
    ...
    hfreshAnchor,
    ⟨ℓ.1, by
      have hfiberNonempty := hfixedSlope.nonempty
      obtain ⟨i, hi⟩ := hfiberNonempty
      change (hits i).length = ℓ.1 at hi
      have hiPos := (step i).hits_nonempty
      change 0 < (hits i).length at hiPos
      omega,
      by
        have := ℓ.isLt
        omega,
...
theorem exactBasis_counterexample_forces_noncontainedRankInjury_or_cofinalFreshPureMatchings
  ...
  · obtain ⟨hcontained, hpure⟩ := hpure
    refine ⟨B, hBA, hBInfinite, hservice,
      Or.inr ⟨hcontained, ?_⟩⟩
    exact
...
theorem zeroNormalized_counterexample_forces_cofinalFixedReservoirPredecessorRankDestruction
    ...
    ∃ C B : Set ℕ,
      ...
      ∀ L, ∃ ℓ n,
        ...
        DestroysAt
          (additiveSupportFamily A (ℓ + 1)) B n ∧
        ∃ D₀ D : Finset ℕ,
          D₀ ⊆ D ∧
          D₀.Nonempty ∧ D.Nonempty ∧
          (D : Set ℕ) ⊆ B ∧
          Disjoint D (Finset.range (L + 1)) ∧
          DestroysAt
            (additiveSupportFamily A ℓ)
            (D₀ : Set ℕ) n ∧
          DestroysAt
  ...
  have hD₀nonempty' : D₀.Nonempty := hD₀nonempty
  ...
  have hcoreB :
      Disjoint (core : Set ℕ) B :=
    hCB.mono_left hcoreC
  have hcoreD₀ :
      Disjoint (core : Set ℕ) (D₀ : Set ℕ) :=
    hcoreB.mono_right hDB
  have hdestroyD₀ :
      DestroysAt
        (additiveSupportFamily A hits.length)
        (D₀ : Set ℕ) hits.sum :=
  ...
  have hcoreD :
      Disjoint (core : Set ℕ) (D : Set ℕ) :=
    hcoreB.mono_right hDBfull
  have horderSucc :
      hits.length + 1 + j = h + 1 := by
    omega
  have hdestroyD :
      DestroysAt
        ...
        (D : Set ℕ) hits.sum :=
    complementarySurvivingCore_forces_hitTargetDestroyer
      hminimalH horderSucc htarget hcoreR hcoreD
  have hdestroySucc :
      DestroysAt
        (additiveSupportFamily A (hits.length + 1))
        B hits.sum :=
    hdestroyD.mono hDBfull
  ...
  exact ⟨hits.length, hits.sum, hhitsPos, hle,
    ...
    D₀, D, hD₀D, hD₀nonempty', hDnonempty,
    hDBfull, hDRange, hdestroyD₀, hdestroyD⟩
...
theorem zeroNormalized_counterexample_forces_fixedPredecessorRankCofinalDestruction
  ...
  rcases hdata i with
    ⟨_hrankPos, _hrankLe, hiTarget,
      ...
      hrepresentedSucc, hdestroySucc, _hnested⟩
...
theorem infiniteUncoveredBlocks_have_infiniteDeletion_preservingStreams
    {A K : Set ℕ} {h : ℕ}
    {cell : ℕ → Finset ℕ}
    {rank rawTarget failure target : ℕ → ℕ}
    {hits : ℕ → List ℕ}
    (P : IsFiniteBlockPartition K cell)
    (hU : Set.Infinite
      {j | ∃ x ∈ cell j, ∀ i, x ∉ hits i})
    (hcoherent : ∀ i,
      ...
      (∀ x ∈ hits i, x ∈ A) ∧
      failure i = (hits i).sum + rawTarget i)
    (hrawSurvival : ∀ s : BlockSelector cell, ∀ i,
      ∃ E ∈ additiveSupportFamily A (rank i) (rawTarget i),
        Disjoint (E : Set ℕ) (selectedSet s))
    ...
    ∃ S : Set ℕ, ∃ s : BlockSelector cell,
      ...
      ∀ i,
        ∃ H ∈ additiveSupportFamily A h (target i),
          Disjoint (H : Set ℕ) S := by
  classical
  let U : Set ℕ :=
    {j | ∃ x ∈ cell j, ∀ i, x ∉ hits i}
  have hchoice : ∀ j, j ∈ U →
      ∃ x, x ∈ cell j ∧ ∀ i, x ∉ hits i := by
    intro j hj
    exact hj
  choose point hpointCell hpointAvoid using hchoice
  let s : BlockSelector cell := fun j =>
    if hj : j ∈ U then
      ⟨point j hj, hpointCell j hj⟩
    else
      ⟨(P.nonempty j).choose, (P.nonempty j).choose_spec⟩
  let S : Set ℕ := (fun j => (s j).1) '' U
  have hU' : U.Infinite := by
    simpa only [U] using hU
  have hSInfinite : S.Infinite := by
    exact hU'.image (P.selector_injective s).injOn
  have hSK : S ⊆ K := by
    rintro x ⟨j, _hjU, rfl⟩
    exact (P.mem_iff _).2 ⟨j, (s j).2⟩
  have hSSelected : S ⊆ selectedSet s := by
    rintro x ⟨j, _hjU, rfl⟩
    exact ⟨j, rfl⟩
  have hhitsS : ∀ i,
      Disjoint ((hits i).toFinset : Set ℕ) S := by
    intro i
    rw [Set.disjoint_left]
    intro x hxHits hxS
    obtain ⟨j, hjU, hjx⟩ := hxS
    have hsPoint : (s j).1 = point j hjU := by
      simp only [s, dif_pos hjU]
    have hsHit : (s j).1 ∈ (hits i).toFinset := by
      simpa only [hjx] using hxHits
    have hpointHit : point j hjU ∈ hits i := by
  ...
  have hraw :
    ...
    obtain ⟨E, hER, hEselected⟩ :=
      hrawSurvival s i
    exact ⟨E, hER,
      Set.disjoint_of_subset_right hSSelected hEselected⟩
  have hfailure :
      ∀ i, ∃ G ∈ additiveSupportFamily A h (failure i),
        Disjoint (G : Set ℕ) S := by
    intro i
    obtain ⟨E, hER, hES⟩ := hraw i
    let G :=
      (hits i).foldr (fun x H => insert x H) E
    have hGRaw :
        G ∈ additiveSupportFamily A
          ((hits i).length + rank i)
          ((hits i).sum + rawTarget i) := by
      exact foldr_insert_mem_additiveSupportFamily
        (hcoherent i).2.1 hER
    have hGR :
        G ∈ additiveSupportFamily A h (failure i) := by
      rw [(hcoherent i).1, ← (hcoherent i).2.2] at hGRaw
      exact hGRaw
    refine ⟨G, hGR, ?_⟩
    rw [Set.disjoint_left]
    intro x hxG hxS
    ...
    rcases Finset.mem_union.mp hxUnion with hxHits | hxE
    · exact Set.disjoint_left.mp (hhitsS i)
        (Finset.mem_coe.mpr hxHits) hxS
    · exact Set.disjoint_left.mp hES
        (Finset.mem_coe.mpr hxE) hxS
  have htarget :
      ∀ i, ∃ H ∈ additiveSupportFamily A h (target i),
        Disjoint (H : Set ℕ) S := by
    intro i
    obtain ⟨H, hHR, hHselected⟩ :=
      htargetSurvival s i
    exact ⟨H, hHR,
...
theorem exactBasis_counterexample_forces_noncontainedRankInjury_or_coherentPureBlockSurvival
  ...
  · right
    obtain ⟨⟨U, hcontained⟩, hpure⟩ := hpure
    ...
    obtain ⟨K, cell, target, failure, rank, rawTarget, hits,
        ...
        hcoherent, hrawSurvival, hhitsForward, hhitGrowth,
    ...
    have hcertificates :
      ...
      by
        ...
        have hQlateN : ∀ q ∈ Q, N ≤ q := by
          intro q hqQ
          exact (le_max_left N (U + 1)).trans
            (hQlate q hqQ)
        have hQinside :
            ∀ q ∈ Q,
              ∀ E ∈ additiveSupportFamily A (h + 1) q,
                ∀ x ∈ E, x ∈ B := by
          intro q hqQ E hER x hxE
          obtain ⟨s, hqDestroy, _hprivate⟩ :=
            hlocalized q hqQ
          have hselectedB : selectedSet s ⊆ B :=
            (P.selectedSet_subset s).trans hKB
          have hqDestroyB :
              DestroysAt
                (additiveSupportFamily A (h + 1)) B q :=
            hqDestroy.mono hselectedB
          have hUq : U < q := by
            have := hQlate q hqQ
            omega
          exact hcontained q hUq hqDestroyB E hER x hxE
        exact ⟨Q, hQnonempty, hQlateN, hcert,
          hlocalized, hQinside, hQsafe⟩
    exact ⟨B, c, K, cell, target, failure, rank, rawTarget, hits,
      hBA, hBInfinite, hservice, ⟨U, hcontained⟩,
      hcA, hKB, hKInfinite,
      hcK, P, htarget, hfailure, hcoherent,
      hrawSurvival, hhitsForward, hhitGrowth,
      hforwardMatchingEndpoint,
      hfailureSurvival,
